var menuWtihoutBp = function(a_group){
    ...
    if(groupByCommand[groupByCommand.length-1].match(/CLOSE\s+\w+/g) !== null){
        closeCursor = groupByCommand[groupByCommand.length-1].match(/CLOSE\s+\w+/g)[0];
        groupByCommand[groupByCommand.length-1] = groupByCommand[groupByCommand.length-1].replace(/CLOSE\s+\w+/g,'');
    }
